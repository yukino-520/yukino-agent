...
| 记忆系统 | `service_club/core/memory/` | SQLite/JSON 持久化；BM25、CJK n-gram、概念、时效、重要度与可选向量混合召回，向量失败自动回退 |
...
| 关系模型 | `service_club/core/companion/relationship.py` | SQLite 持久化信任、互动质量、支持证据、明确边界、破裂/修复；用证据门槛生成 new/warming/familiar/trusted |
...
| 路由反馈 | `service_club/core/conversation/route_feedback.py` | SQLite 持久化成功/失败与延迟，维护 Beta belief，并参与活动室自适应路由 |
| 自诊断 | `service_club/core/runtime/doctor.py` | 输出 memory/tools/agents/router/degradation 健康报告 |
| 心理状态 | `service_club/core/companion/mental_state.py` | 将每轮情绪写入 SQLite，按会话恢复主导情绪、低落连续度和 follow-up 信号 |
...
硬优先级始终是：`@角色` > solo 所选角色 > manual 所选角色 > adaptive 评分。数据库不可写时，反馈存储随 `MemoryManager` 降级到临时库；反馈不可用时，先验成功率回到 0.5，语义路由仍然工作。
...
记忆在 `service_club/core/memory/store.py`，使用 SQLite。
...
3. SQLite 删除当前 session 下内容包含 query 的显式记忆。
...
6. 工具结果写入 `tool_results`：
   - `audit.deleted_count` 记录 SQLite 显式记忆删除数。
...
降级规则：关系库没有记录时返回保守的 `new/stable`；模型忽略破裂修复提示时，`ResponsePolicyEnforcer` 在最终文本前补充承认影响与道歉；数据库不可写则沿用 MemoryManager 的临时 SQLite 回退。
...
- 如果项目目录数据库不可写，自动切换到 `/tmp/service_club_<pid>.sqlite3`。
- 如果记忆写入失败，不应该让聊天整体崩溃。
...
`ContextWindowManager` 默认总预算为 8000 个估算 token，其中预留 2500 给 system prompt、人格、记忆和工具上下文。消息历史可使用约 5500 token，默认保留最近 8 条消息的完整文本；更早历史被压缩为最多 700 token 的确定性摘要，并写入 SQLite `context_checkpoints`。
...
降级与关闭：设置 `AGENT_QUALITY_GUARD_ENABLED=false` 可关闭审计，此时原文直接进入最终安全策略层；SQLite 不可写时沿用 `MemoryManager` 的临时库回退，当前轮确定性修复仍工作；案例统计不可用不应阻断聊天，只会失去跨轮质量教训。Doctor 的 `response_quality` 项会返回启用状态、维度和案例计数。
...
降级规则：`AGENT_INTERACTION_LEARNING_ENABLED=false` 时完全关闭观察和快照；SQLite 读取或写入失败时返回 `store_unavailable`，聊天继续，只失去结果学习；确认清空记忆时删除策略快照、结果事件和质量案例。Doctor 的 `interaction_learning` 会显示开关、证据门槛、24 小时归因窗口、30 天教训时效和存储状态。
...
降级顺序固定：`high_risk/crisis` 不创建或更新检查点，但可读取旧状态且安全规则始终优先；没有历史时不注入任何自我模型；SQLite 不可写时沿用 `MemoryManager` 的临时数据库回退；自我模型异常不应该改变模型和纯文本回复的基础降级路线。确认清空 session 记忆时同步删除检查点，避免用户清空后 Agent 仍通过成长状态间接记得关系。
...
状态保存在 SQLite `affective_states` 表，字段包含 label、intensity、P/A/D 和更新时间。数据库不可写时沿用 MemoryManager 的 `/tmp` 降级，不阻断聊天。`/api/mental-state`、`/api/memory-state`、`companion_state` 和模型 `tool_context` 都返回或注入同一份状态。
